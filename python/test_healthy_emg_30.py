...
def main():
    ...
    detector = EMGAnomalyDetector(
        "models/emg_lstm_vae_20251226_101048/best_model.h5"
    )
    
    # 加载健康肌电数据
    print("加载健康肌电数据...")
    health_data = np.load("healthy_muscle_emg_right_swing.npz")
    ...
    # 加载评估结果（用于获取阈值）
    print("\n加载训练集评估结果...")
    eval_data = np.load("models/emg_lstm_vae_20251226_101048/evaluation_results.npz")
